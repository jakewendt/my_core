...
/*
var libs = "aes,aesprng,entropy,armour,lecuyer,md5,utf-8".split(",")
for( lib in libs )
	document.write("<script language='JavaScript' type='text/javascript' src='"+libs[lib]+".js'><\/script>");
*/

function getPassword(confirmation){
	password = prompt("Please enter your password");
	if( password ) {
		if( typeof(confirmation) != 'undefined' && confirmation == true ) {
			password_confirmation = prompt("Please confirm your password");
...
/*
	...
	setKey  --  Prompt for password string and set key
...
function setKey(incoming_key) {
	...
	key =  hexToByteArray(kmd5e + kmd5o);
// key is like 
// 5,26,153,17,222,123,91,188,97,11,118,244,237,168,52,160,182,187,67,223,69,37,185,40,161,5,251,87,65,189,219,234
//	var hs = kmd5e + kmd5o;
...
/*
	...
	Encrypt incoming_text and return encrypted text

*/
function Encrypt_text(incoming_text) {
	var v, i;
	var prefix = "#####  Encrypted: decrypt here or with http://www.fourmilab.ch/javascrypt/\n",
...
//	document.plain.text.value = prefix + v + suffix;
...
function determineArmourType(s) {
	...
	if (pmin < s.length) {
		...
		// my guess is that if it gets here, we're screwed
		alert("Failure to determine Armour Type.\nDecryption halted.");
		kt = null;	// don't know if I should do this.
...
/*
	...
	Decrypt incoming_text and return decrypted text

*/
function Decrypt_text(incoming_text) {
...
function Encrypt(element){
	if( element.value ) {
		if( encrypted_text = Encrypt_text(element.value) ){
			element.value = encrypted_text;
		}
	} else if( element.innerHTML ) {
		if( encrypted_text = Encrypt_text(element.innerHTML) ){
			element.innerHTML = encrypted_text;
		}
	} else {
		alert("Element doesn't have a value or innerHTML attribute.");
	}
}

function Decrypt(element){
	if( element.value ) {
		if( decrypted_text = Decrypt_text(element.value) ){
			element.value = decrypted_text;
		}
	} else if( element.innerHTML ) {
		if( decrypted_text = Decrypt_text(element.innerHTML) ){
			// escape, encodeURI, encodeURIComponent don't work
			// I really don't know why I need to add the C on both 
			// the regex and replacement


			// I don't know if escapeHTML is JS or Prototype.  I think that its prototype
//			element.innerHTML = decrypted_text.escapeHTML();	//replace(/<C/,"&lt;C");
			element.innerHTML = decrypted_text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
...
function Encrypt_ID(id){
	Encrypt(document.getElementById(id));
}

function Decrypt_ID(id){
	Decrypt(document.getElementById(id));
}
